...
在浏览区新增缩略图尺寸调整功能，支持小、中、大三种显示尺寸。小尺寸 160px，中尺寸 240px，大尺寸 320px。默认显示中尺寸。尺寸调整按钮放置于排序栏右侧，与排序控件平行。
...
- **中尺寸**: 缩略图最小显示宽度 240px
- **大尺寸**: 缩略图最小显示宽度 320px
- **尺寸调整按钮**: 用于切换缩略图尺寸的交互控件，放置于排序栏右侧
...
### Requirement 3: 尺寸调整按钮位于排序栏右侧

**User Story:** AS 用户, I want 尺寸调整按钮位于排序栏右侧、与排序控件平行, so that 与排序操作集中在同一区域便于操作

#### Acceptance Criteria

1. WHEN 排序栏可见, 系统 SHALL 在排序栏右侧展示尺寸调整按钮
2. WHILE 排序栏展示按钮, 系统 SHALL 将按钮与排序控件置于同一行
