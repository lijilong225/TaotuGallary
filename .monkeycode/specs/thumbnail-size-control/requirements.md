...
在浏览区新增缩略图尺寸调整功能，支持小、中、大三种显示尺寸。中尺寸比小尺寸宽 100%，大尺寸比中尺寸再宽 100%。默认显示中尺寸。尺寸调整按钮放置于页面右侧。

## Glossary

- **缩略图尺寸**: 图片宫格中单个缩略图卡片的最小显示宽度，分为小、中、大三档
- **小尺寸**: 当前缩略图默认宽度 160px
- **中尺寸**: 小尺寸基础上增加 100%，即 320px
- **大尺寸**: 中尺寸基础上再增加 100%，即 640px
...
3. WHEN 尺寸为中, 系统 SHALL 以 320px 为最小宽度渲染缩略图卡片
4. WHEN 尺寸为大, 系统 SHALL 以 640px 为最小宽度渲染缩略图卡片
...
1. IF 当前尺寸为大, 系统 SHALL 请求并展示宽度不小于 640px 的缩略图
2. IF 当前尺寸为中, 系统 SHALL 请求并展示宽度不小于 320px 的缩略图
